...
# ─── CONFIGURACIÓN ────────────────────────────────────────────────────────────
PLANILLA_BASE        = "fc_cuit_aamm_aamm_razonsocial_v1_0.xlsx"
...
def codigo_desde_nombre(tipo_word, letra):
    ...
    LIMITACIÓN CONOCIDA: las notas de crédito/débito de EXPORTACIÓN (códigos
    020/021) se clasificarían como Factura E (019), porque el chequeo de
    'EXPORTACION' está antes que el de crédito/débito. El Amarillo no exporta,
    así que hoy no afecta. Si algún día hay exportaciones, mover el bloque
    de EXPORTACION después de los chequeos es_credito/es_debito y mapear:
    crédito exportación -> '021', débito exportación -> '020'."""
...
def leer_items_xls(ruta_xls):
    ...
    if "NETO1" not in df.columns or "IVA1" not in df.columns:
        print(f"  [AVISO] {os.path.basename(ruta_xls)} sin columnas NETO1/IVA1.")
        return None
    ...
    for _, item in items.iterrows():
        ...
        # Validar NETO1 e IVA1: un NaN acá se colaría silencioso en la planilla.
        if pd.isna(item["NETO1"]) or pd.isna(item["IVA1"]):
            print(f"  [ERROR] {os.path.basename(ruta_xls)}: ítem "
                  f"'{descrip[:40]}' con NETO1/IVA1 vacío. Comprobante omitido "
                  f"(se cargará solo la cabecera). Revisar el XLS.")
            return None
...
def _clave_desde_celdas(tipo, pto, nro):
    """Normaliza (tipo, pto, nro) leídos de la planilla a la misma forma que
    usa el script: tipo como texto de 3 dígitos ('001'), pto y nro como int.
    Es robusto ante celdas guardadas como número (1, 3.0) o texto ('001').
    Devuelve None si la fila no tiene una clave válida."""
    try:
        t = str(tipo).strip()
        t = f"{int(float(t)):03d}"          # 1, '1', '001', 1.0 -> '001'
        p = int(float(str(pto).strip()))
        n = int(float(str(nro).strip()))
        return (t, p, n)
    except (ValueError, TypeError):
        return None
...
def main():
    ...
    for row in ws.iter_rows(min_row=2):
        if row[4].value is not None:
            clave = _clave_desde_celdas(row[2].value, row[3].value, row[4].value)
            if clave is not None:
                existentes.add(clave)

    # Última fila con datos (referencia de estilo).
    # Si la planilla base está vacía (template oficial), se usa la fila 2:
    # viene pre-formateada por AFIP y sirve como referencia de estilo.
    fila_ref = 2
    for row in ws.iter_rows(min_row=2):
        if row[4].value is not None:
            fila_ref = row[0].row
    ...
    for row in ws.iter_rows(min_row=2):
        if row[4].value is None:
            continue
        clave = _clave_desde_celdas(row[2].value, row[3].value, row[4].value) \
                or (str(row[2].value), row[3].value, row[4].value)
    ...
    # Última fila que tenía datos ANTES de reescribir (para limpiar sobrantes)
    fila_max_previa = 1
    for row in ws.iter_rows(min_row=2):
        if row[4].value is not None:
            fila_max_previa = row[0].row
    ...
    # Limpiar restos: si la versión anterior tenía filas más allá del final
    # nuevo (separadores, comprobantes borrados a mano, etc.), vaciarlas para
    # que no queden datos viejos huérfanos al final de la hoja.
    if fila_max_previa >= fila:
        for f in range(fila, fila_max_previa + 1):
            for col in range(1, 26):
                ws.cell(row=f, column=col).value = None
        print(f"  (Se limpiaron {fila_max_previa - fila + 1} fila(s) "
              f"sobrantes de la versión anterior.)")
